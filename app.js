import { LIVE_SITE_URL, analyzeSearch, buildListingSearchQueries, createFeedbackRecord, matchStaticSearchResults, parseListingsFromText, searchResultsToListingText } from './shortlist.js';
    ...
    <section class="panel input-panel" aria-labelledby="needs-heading">
      ...
      <form id="needs-form" class="grid-form">
        ...
        <div class="actions full">
          ...
          <button id="auto-search" class="secondary" type="button">Use pre-generated search snippets if available</button>
          <p class="hint">GitHub Pages cannot run live per-user SerpApi searches. If the owner refreshed public static snippets with GitHub Actions, this button can load those pre-generated leads. Otherwise, use the targeted links and paste listing text below.</p>
        </div>
      </form>
    </section>
  ...
  panel.innerHTML = `
    <div class="section-heading">
      <div>
        ...
        <h2>Use static search snippets, targeted links, or pasted text</h2>
        <p class="hint">SerpApi runs only in GitHub Actions with the repository secret. Any loaded snippets are pre-generated public data for GitHub Pages — not live results for this renter. The targeted links and paste fallback remain the source of renter-specific search.</p>
...
async function runAutomaticSearch() {
  searchStatus = 'Checking for pre-generated static search snippets…';
  renderSearchPlan();
  const button = document.querySelector('#auto-search');
  if (button) button.disabled = true;
  try {
    const response = await fetch('./generated/search-results.json', { cache: 'no-store' });
    if (!response.ok) {
      throw new Error('No pre-generated search-result file is deployed yet.');
    }
    const payload = await response.json();
    const matches = matchStaticSearchResults(state, payload.results || []);
    const listingText = searchResultsToListingText(matches);
    if (!listingText) throw new Error('The pre-generated snippets did not include usable listing text.');
    ...
    const generated = payload.generatedAt ? ` generated ${new Date(payload.generatedAt).toLocaleDateString()}` : '';
    searchStatus = `Loaded ${matches.length} public static snippets${generated}. These were pre-generated by GitHub Actions and are not live per-user search results; verify each listing and use the targeted links for renter-specific search.`;
    renderSearchPlan();
    runAnalysis();
  } catch (error) {
